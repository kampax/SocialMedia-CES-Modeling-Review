...
# Create a column with the desired format for authors
a$author <- sapply(1:nrow(a), function(i) {
  # Split the author string by comma or semicolon
  authors <- unlist(strsplit(a$Authors[i], ",|;"))
  # Remove blank spaces
  authors <- trimws(authors)
  
  # Extract only last names
  authors <- authors[seq(1, length(authors), by = 2)]
  
  # Logic for managing the number of authors
  if (length(authors) == 1) {
    # A single author
    authors[1]
...
#Add the year of publication to the final format
...
# Read the specific 'Evaluation Metrics' sheet
eval_metric_values <- read_sheet(url, sheet = "EvaluationMetrics")
...
y<- ggplot(publication_year, aes(Publication.Year, n, group = 1))+
  ...
  scale_x_continuous(breaks = c(2018,2022), limits = c(2015, 2024))
...
map_literature_review <- ggplot(df_plot) + 
  geom_sf(aes(fill = cut(count, breaks = c(0,3,6,9,15), 
                         labels = c("1-3","4-7", "7-10", "10-12")))) +
...
length(unique(d$ID[d$Relevant == "no" | d$Spatial == "no"]))
...
length(unique(d$ID[d$Includes.models =="no"]))
